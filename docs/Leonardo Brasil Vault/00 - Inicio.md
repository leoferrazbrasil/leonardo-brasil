---
title: Leonardo Brasil - Headquarters
tags:
  - dashboard
  - home
date: 2026-07-20
---

# 🧠 Leonardo Brasil - Knowledge Vault

Bem-vindo ao cofre estratégico do projeto de marca pessoal **Leonardo Brasil** (`leonardobrasil.com.br`). Este repositório centraliza toda a inteligência de negócios, marketing, desenvolvimento e gestão de produto.

## 🗂️ Estrutura de Pastas (P.A.R.A Method)

### [[1. Gestão e Produto]]
Diretrizes gerais sobre o escopo da "Consultoria de Estrutura de Vendas", precificação, onboarding de clientes e planejamento de expansão.
- **Nova oferta (2026-07-21):** [[Consultoria_Comercial_Estrategica]] — segunda linha de produto, divulgada via faixa de anúncio global no site.

### [[2. Engenharia]]
Documentação técnica do site, decisões de arquitetura (Vite, React, Tailwind), deploy e scripts customizados.

### [[3. Design e Experiencia]]
Brandbook da marca pessoal, guias de estilo (cores, tipografia), UI/UX e tom de voz da comunicação.

### [[4. Marketing e Growth]]
Estratégias de aquisição de clientes (Inbound e Outbound).
- **SEO & Topic Clusters:** [[Estrategia_Global_SEO]]
- **Tráfego Pago:** Planejamento de campanhas Meta/Google Ads.
- **Copywriting:** Dicionário de dores e mapeamento de personas (High-Ticket B2B).

---

> [!TIP]
> Use a busca do Obsidian (`Ctrl/Cmd + O`) ou pesquise por `#tags` para encontrar rapidamente estratégias documentadas.
